...
export class Hyp3eDice {
  ...
  static buildAttackFormula(rollData, itemData, ammoData = null, actorData = null) {
    ...
    atkRollParts = tmpAtkRollParts.map(str => {
      const s = str.trim();
    ...
    Hyp3eLogger.info("Hyp3eDice buildAttackFormula", "Debug attack roll parts:", debugAtkRollParts);
    ...
    if (itemData.atkMod && parseInt(itemData.atkMod) != 0) {
      if (atkRollParts.length > 1) {
        ...
        debugAtkRollParts.splice(1, 0, `<tr><td>Item Atk Mod</td><td>${itemAtkMod}</td></tr>`);
      } else {
        atkRollParts.push(itemData.atkMod);
        debugAtkRollParts.push(`<tr><td>Item Atk Mod</td><td>${itemAtkMod}</td></tr>`);
      }
    }

    // Apply the ammunition attack mod if needed
    if (ammoData?.atkMod) {
      const ammoAtkMod = parseInt(ammoData.atkMod) > 0 ? `+${parseInt(ammoData.atkMod)}` : `${parseInt(ammoData.atkMod)}`;
      if (atkRollParts.length > 1) {
        atkRollParts.splice(2, 0, ammoData.atkMod);
        debugAtkRollParts.splice(2, 0, `<tr><td>Ammo Atk Mod</td><td>${ammoAtkMod}</td></tr>`);
      } else {
        atkRollParts.push(ammoData.atkMod);
        debugAtkRollParts.push(`<tr><td>Ammo Atk Mod</td><td>${ammoAtkMod}</td></tr>`);
      }
    }
    ...
    Hyp3eLogger.info("Hyp3eDice buildAttackFormula", "Attack roll parts:", atkRollParts);
    Hyp3eLogger.info("Hyp3eDice buildAttackFormula", "Debug attack roll parts:", debugAtkRollParts);
  ...
  static buildDamageFormula(itemData, ammoData = null, actorData = null) {
    ...
    if (itemData.damage2h) {
      if (debugDmgRoll2Parts[1].match(faRegex) > "") {
        dmgRoll2Parts[0] = dmgRoll2Parts[0].replace(faRegex, `+ ${actorData.fa}`);
