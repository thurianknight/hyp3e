...
export class HYP3EConditionEditor extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  async _prepareContext(options) {
    ...
    context.condition = foundry.utils.deepClone(cond);
    context.tests = context.condition.tests;
  ...
  static async #delTest(event, target) {
    ...
    await this.effect.setFlag("hyp3e", "condition", cond);
    this.render();
  }
  ...
  static async onSubmit(event, form, formData) {
    ...
    for (let i = 0; i < testCount; i++) {
      condition.tests.push({
        ...
        // value: HYP3EConditionEditor.parseValue(formData.object[`value-${i}`])
      });
    }

    // await app.effect.update({"flags.hyp3e.condition": condition});
    await app.effect.setFlag("hyp3e", "condition", condition);
